 ...
 * @param {MouseEventHandler} onClick Eventhandler for the button.
 * 
 * @returns {JSX.Element} A button to use in sidebar.
 */
const NavigationButton = ({ text, variant, icon, onClick }: NavigationButtonProps): JSX.Element => {

	let navigationButton;

	// Chooses the right css style for button
	const buttonClassNames = icon === 'logout' ?
		classNames(classes.button, classes.iconText, classes.logout) :
		classNames(classes.button, classes.iconText);

	// Chooses the right css style for button text
